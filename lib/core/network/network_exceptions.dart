...
abstract class NetworkExceptions with _$NetworkExceptions {
  ...
  static NetworkExceptions handleResponse(Response? response) {
    ...
    String allErrors = listOfErrors.map((e) => "${e.statusCode} : ${e.message}  ").toString().replaceAll("(", "").replaceAll(")", "");
    int statusCode = response?.statusCode ?? 0;
    switch (statusCode) {
      case 401:
        ErrorModel errorModel = ErrorModel.fromJson(response?.data);
        return NetworkExceptions.defaultError("${errorModel.message}");
      case 400:
      case 403:
      ...
      default:
        ...
        return NetworkExceptions.defaultError(
          "Received invalid status code: ${response?.statusMessage}",
        );
    }
  }
  ...
  static NetworkExceptions getDioException(error) {
    if (error is Exception) {
        ...
        if (error is DioException) {
          switch (error.type) {
            ...
            case DioExceptionType.sendTimeout:
              ...
              break;
            default:
              networkExceptions = const NetworkExceptions.defaultError("error.message!");
              break;
          }
      ...
      if (error.toString().contains("is not a subtype of")) {
        ...
        print("This is from network exceptions");
  ...
  static String getErrorMessage(NetworkExceptions networkExceptions) {
    ...
    networkExceptions.when(notImplemented: () {
      ...
      errorMessage = "Data is not formatted well";
    }, notAcceptable: () {
      errorMessage = "Not acceptable";
    }, badCertificate: () {
      errorMessage = "SSL Certificate not valid";
    });
    return errorMessage;
  }
}
